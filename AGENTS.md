...
## Purpose
This ROS 2 package is the AMRL Cobot implementation of CodeBotler's robot action contract. It exposes the action servers that `../codebotler/robot_client.py` calls, and bridges those actions to AMRL navigation, OpenAI VLM perception, speech, GUI ask/response, pick, and place topics/services.
...
- `actions.py --vlm-model <model>` sets the OpenAI vision model used by `is_in_room`; default is `gpt-5-mini`.
- `go_to` normalizes requested names by lowercasing, stripping `"'s"`, and replacing hyphens with spaces, then looks up coordinates in `data.yaml`, publishes `amrl_msgs/Localization2DMsg` goals, and watches navigation status.
...
- Action cancellation is supported where it has a real local effect: `go_to` publishes `NAV_RESET_TOPIC`, `ask` clears the GUI prompt/response wait, fast read actions cancel before success, and `say` reports cancel after speech returns. Pick/place cancellation is intentionally rejected until downstream cancel topics or actions are wired.
...
- OpenAI credentials are loaded from repo-root `.openai_api_key` first, then `OPENAI_API_KEY`. Do not commit the key file.

## Key Files
- `data.yaml`: source of topic names, thresholds, map selection, distance threshold, and named locations.
...
- Launch through `tmux/codebotler`, or run `python3 actions.py` and `python3 gui.py` from this package after the AMRL/Cobot stack, camera, navigation topics, `cobot_codebotler_actions`, pick/place status topics, OpenAI credentials, and audio devices are available.
...
- Action clients block on goal acceptance and then on the final result unless cancellation is requested. Feedback fields remain unused.
...
- Avoid committing `.openai_api_key`, generated `images/`, audio files, or local output artifacts.
